...
SML tags available:
    ...
    headless_group.add_argument(
        cli_options[5],
        type=str,
        help="""Path to the ebook file for conversion. Cannot be used when --ebooks_dir or --text is present.""",
    )
    headless_group.add_argument(
        cli_options[6],
        type=str,
    ...
    headless_group.add_argument(
        cli_options[7],
        type=str,
        help="""Raw text for conversion. Cannot be used when --ebook or --ebooks_dir is present.""",
    )
    headless_group.add_argument(
        cli_options[8],
        type=str,
    ...
    headless_optional_group.add_argument(
        ...
        default=None,
        metavar="ISO3",
        help="""(Optional) Translate ebook to a target language (ISO 639-3 code, e.g. eng, fra, deu) before TTS synthesis.
    ...
    headless_optional_group.add_argument(
        cli_options[18],
        type=float,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["temperature"],
    ...
    headless_optional_group.add_argument(
        cli_options[19],
        type=float,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["length_penalty"],
    ...
    headless_optional_group.add_argument(
        cli_options[20],
        type=int,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["num_beams"],
    ...
    headless_optional_group.add_argument(
        cli_options[21],
        type=float,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["repetition_penalty"],
    ...
    headless_optional_group.add_argument(
        cli_options[22],
        type=int,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["top_k"],
    ...
    headless_optional_group.add_argument(
        cli_options[23],
        type=float,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["top_p"],
    ...
    headless_optional_group.add_argument(
        cli_options[24],
        type=float,
        default=default_engine_settings[TTS_ENGINES["XTTS"]]["speed"],
    ...
    headless_optional_group.add_argument(
        cli_options[30], action="store_true", help=argparse.SUPPRESS
    )
    headless_optional_group.add_argument(
        cli_options[31], action="store_true", help=argparse.SUPPRESS
    )
    headless_optional_group.add_argument(
        cli_options[33],
        action="store_true",
        help="""(Optional) Enable Audiobookshelf upload after conversion. Requires --abs_server_url, --abs_api_token, --abs_library_id (or .env file).""",
    )
    headless_optional_group.add_argument(
        cli_options[34],
        type=str,
        default=None,
        help="""(Optional) Audiobookshelf server URL, e.g. http://192.168.1.100:13378""",
    )
    headless_optional_group.add_argument(
        cli_options[35],
        type=str,
        default=None,
        help="""(Optional) Audiobookshelf API token from user settings.""",
    )
    headless_optional_group.add_argument(
        cli_options[36],
        type=str,
        default=None,
        help="""(Optional) Audiobookshelf library ID (e.g. lib_abc123).""",
    )
    headless_optional_group.add_argument(
        cli_options[37],
        action="store_true",
        help="""(Optional) Output one m4b file per chapter instead of a single file.""",
    )
    headless_optional_group.add_argument(
        cli_options[38],
        action="store_true",
        help="""(Optional) Force full rebuild, ignoring existing output.""",
    )
    ...
    if not "help" in args:
        ...
        if args["script_mode"] == FULL_DOCKER and not is_running_in_container():
            ...
            print(error)
            sys.exit(1)
        elif (not check_virtual_env(args["script_mode"])) or (
            not check_python_version(args["script_mode"])
        ):
            sys.exit(1)
        ...
        if args.get("headless", False):
            ...
            args["output_chapter_mode"] = bool(args.get("chapter_mode", False))
            args["output_overwrite"] = bool(args.get("output_overwrite", False))
            args["abs_enabled"] = (
                bool(args.get("abs_enabled", False)) or default_abs_enabled
            )
            args["abs_server_url"] = (
                args.get("abs_server_url") or default_abs_server_url
            )
            args["abs_api_token"] = args.get("abs_api_token") or default_abs_api_token
            args["abs_library_id"] = (
                args.get("abs_library_id") or default_abs_library_id
            )
            ...
            if _user_translate_raw:
                ...
                if tgt == args.get("language"):
                    print(
                        f"[translate] target equals source ({tgt}), translation skipped."
                    )
                else:
                    try:
            ...
            # -------------------------------------------------------------------------
            specified_input = sum(
                args.get(k, None) is not None for k in ("ebook", "ebooks_dir", "text")
            )
                    ...
                    if not os.path.exists(args["ebooks_dir"]):
                        ...
                        if args.get("voice_map"):
                            ...
                            if not os.path.exists(voice_map_path):
                                ...
                                try:
                                    with open(
                                        voice_map_path, "r", encoding="utf-8"
                                    ) as f:
                                        raw = json.load(f)
                                    if not isinstance(raw, dict):
                                        ...
                                        for k, v in raw.items():
                                            ...
                                            voice_map[normalized_key] = (
                                                os.path.abspath(v) if v else None
                                            )
                        ...
                        if not error:
                            ...
                            for name in all_entries:
                                abs_path = os.path.abspath(
                                    os.path.join(args["ebooks_dir"], name)
                                )
                            ...
                            if not args["ebook_list"]:
                                ...
                                for file in ebook_list:
                                    c.context.sessions[args["id"]]["status"] = (
                                        c.status_tags["READY"]
                                    )
                                    ...
                                    override = voice_map.get(file) or voice_map.get(
                                        os.path.basename(file)
                                    )
                                    if override and not os.path.exists(override):
                                        print(
                                            f"--voice_map: override for {Path(file).name} ({override}) not found, falling back to --voice"
                                        )
                                        override = None
