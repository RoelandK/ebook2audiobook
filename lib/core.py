...
ebook_modes = {"SINGLE": "single", "DIRECTORY": "directory", "TEXT": "text"}

save_session_keys_except = ["blocks_orig", "blocks_saved", "blocks_current"]

file_prefixes = {"clone": "__", "saved": "__saved_", "current": "__current_"}
        ...
        self.sessions[session_id] = self._recursive_proxy(
            {
                ...
                ####### Xtts settings
                "xtts_temperature": default_engine_settings[TTS_ENGINES["XTTS"]][
                    "temperature"
                ],
                # "xtts_codec_temperature": default_engine_settings[TTS_ENGINES['XTTS']]['codec_temperature'],
                "xtts_length_penalty": default_engine_settings[TTS_ENGINES["XTTS"]][
                    "length_penalty"
                ],
                "xtts_num_beams": default_engine_settings[TTS_ENGINES["XTTS"]][
                    "num_beams"
                ],
                "xtts_repetition_penalty": default_engine_settings[TTS_ENGINES["XTTS"]][
                    "repetition_penalty"
                ],
                ...
                "xtts_enable_text_splitting": default_engine_settings[
                    TTS_ENGINES["XTTS"]
                ]["enable_text_splitting"],
                ####### Bark settings
                "bark_text_temp": default_engine_settings[TTS_ENGINES["BARK"]][
                    "text_temp"
                ],
                "bark_waveform_temp": default_engine_settings[TTS_ENGINES["BARK"]][
                    "waveform_temp"
                ],
                ####### Audiobook editor
                "audiobook": None,
        ...
        try:
            ...
            if model_path is not None:
                ...
                extractor = VoiceExtractor(
                    session, voice_ref, voice_name, final_voice_file
                )
...
def hash_proxy_dict(proxy_dict: Any) -> str:
    try:
        data = {
            k: v
            for k, v in dict(proxy_dict).items()
    ...
    try:
        ...
        try:
            data = pytesseract.image_to_data(
                img, lang=lang, output_type=pytesseract.Output.DATAFRAME
            )
        ...
        for i, p in enumerate(merged_lines):
            ...
            if p.isupper() and len(p.split()) <= 8:
                ...
                is_heading = True
            elif (i == 0 or (i > 0 and merged_lines[i - 1] == "")) and len(
                p.split()
            ) <= 10:
                is_heading = True
            if is_heading:
    ...
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            if stamp_row is None:
                return {}
            page, block_resume, sentence_resume, voice, tts_engine, fine_tuned = (
                stamp_row
            )
            sentences_by_block = {}
            for block_id, text in conn.execute(
                "SELECT block_id, text FROM sentences ORDER BY block_id, idx"
            ):
                sentences_by_block.setdefault(block_id, []).append(text)
            blocks = []
            for row in conn.execute(
                "SELECT id, expand, keep, text, voice, tts_engine, fine_tuned FROM blocks ORDER BY idx"
            ):
...
def save_db_blocks(session_id: str) -> None:
    try:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            if removed:
                conn.executemany(
                    "DELETE FROM blocks WHERE id=?", [(rid,) for rid in removed]
                )
...
def normalize_epub_zip(session_id: str, file_input: str) -> str | None:
    try:
            ...
            if nested_epub:
                ...
                if not root_mimetype:
                    ...
                    cands = [
                        d
                        for d in dirs
                        if f"{d}/mimetype" in names
                        and f"{d}/META-INF/container.xml" in names
                    ]
                    if len(cands) != 1:
        ...
        session["epub_path"] = os.path.join(
            session["process_dir"], f"__{session['filename_noext']}.epub"
        )
            ...
            if file_ext == ".txt":
                ...
                if xhtml_pages:
                    ...
                    file_input = os.path.join(
                        session["process_dir"], f"{filename_noext}.xhtml"
                    )
                ...
                for i, slide in enumerate(prs.slides):
                    ...
                    if slide_texts:
                        xhtml_content = "\n".join(
                            f"<p>{html_escape(t)}</p>" for t in slide_texts
                        )
                    elif slide_images:
                    ...
                    for rel in docx_doc.part.rels.values():
                        if "image" in rel.reltype:
                            try:
                                ...
                                xhtml_content, error = ocr2xhtml(
                                    img, session["language"]
                                )
                                if xhtml_content:
                                    xhtml_pages.append(xhtml_content)
                                else:
                ...
                if xhtml_pages:
                    ...
                    return False
            msg = (
                f"Running command: {ebook_convert} {file_input} {session['epub_path']}"
            )
            print(msg)
            cmd = [
                ebook_convert,
    ...
    try:
        ...
        if session and session.get("id", False):
            ...
            cover_path = os.path.join(
                session["process_dir"], session["filename_noext"] + ".jpg"
            )
...
*******************************************************************************
        ...
        if session and session.get("id", False):
            ...
            all_docs = [
                item
                for item in epubBook.get_items_of_type(ebooklib.ITEM_DOCUMENT)
                ...
                try:
                    ...
                    if stanza_nlp:
                        ...
                        use_gpu = (
                            ...
                            if (
                                (
                                    session["device"] == devices["CUDA"]["proc"]
                                    and devices["CUDA"]["found"]
                                )
                                or (
                                    session["device"] == devices["ROCM"]["proc"]
                                    and devices["ROCM"]["found"]
                                )
                                or (
                                    session["device"] == devices["XPU"]["proc"]
                                    and devices["XPU"]["found"]
                                )
                                or (
                                    session["device"] == devices["JETSON"]["proc"]
                                    and devices["JETSON"]["found"]
                                )
                            )
                            else False
                        )
                        ...
                        try:
                            ...
                            stanza_has_mwt = "mwt" in stanza_resources.get(
                                stanza_lang, {}
                            )
                        except Exception:
                            pass
                        stanza_processors = (
                            "tokenize,mwt,ner" if stanza_has_mwt else "tokenize,ner"
                        )
            ...
            non_text_filter = NonTextFilter(
                sml_pattern=SML_TAG_PATTERN, lang=session["language"]
            )
            try:
...
def filter_blocks(
    ...
    zip_names: set = None,
    zip_basenames: dict = None,
) -> str | None:

    def _tuple_row(
        node: Any, last_text_char: str | None = None, in_heading: bool = False
    ) -> Generator[tuple[str, Any], None, None] | None:
        try:
                ...
                if isinstance(child, NavigableString):
                    ...
                    if name in heading_tags:
                        ...
                        if title:
                            ...
                            for inner in _tuple_row(
                                child, last_text_char, in_heading=True
                            ):
                                yield inner
                        ...
                        if name in proc_tags:
                            ...
                            if (
                                prev_child_had_data
                                and name in break_tags
                                and not in_heading
                            ):
                                yield ("break", sml_token("break"))
                            for inner in _tuple_row(
                                child, last_text_char, in_heading=in_heading
                            ):
                                return_data = True
                                yield inner
                                if (
    ...
    try:
        ...
        if session and session.get("id", False):
            ...
            raw_html = (
                doc_body.decode("utf-8") if isinstance(doc_body, bytes) else doc_body
            )
            ...
            if not body.get_text(strip=True):
                ...
                if images and zf:
                    ...
                    for img_tag in images:
                        ...
                        if not img_ref:
                            continue
                        img_zip_path = os.path.normpath(
                            os.path.join(doc_dir, img_ref)
                        ).replace("\\", "/")
            ...
            for typ, payload in tuples_list:
                if typ == "heading":
                    ...
                    if prev_typ != typ:
                        token = sml_token(typ)
                        if text_list and text_list[-1] not in {
                            v["static"] for v in TTS_SML.values() if "static" in v
                        }:
            ...
            break_between_alnum_re = re.compile(
                rf"(?<=[\w]){break_token}(?=[\w])", flags=re.UNICODE
            )
            ...
            if stanza_nlp:
                ...
                if re_num.search(text) and (
                    re_ordinal.search(text) or re_year.search(text)
                ):
                    ...
                    if date_spans:
                        ...
                        for start, end, date_text in date_spans:
                            ...
                            processed = re.sub(
                                r"\b\d{4}\b",
                                lambda m: year2words(
                                    m.group(), lang, lang_iso1, is_num2words_compat
                                ),
                                date_text,
                            )
    ...
    def _segment_ideogramms(text: str) -> list[str]:
        ...
        try:
            if lang in ["yue", "yue-Hant", "yue-Hans", "zh-yue", "cantonese"]:
                ...
                result.extend(
                    [t for t in word_tokenize(text, engine="newmm") if t.strip()]
                )
            else:
    ...
    def _join_ideogramms(idg_list: list[str]) -> str:
        try:
            ...
            for token in idg_list:
                ...
                if buffer:
                    if (prev_latin and (cur_starts_latin or cur_starts_nonlatin)) or (
                        prev_nonlatin and cur_starts_latin
                    ):
                        buffer += " "
    ...
    try:
        ...
        for seg_type, seg_content in segments:
            ...
            if 0 < cut_idx <= len(combined):
                ...
                if (
                    seg_clean_len <= max_chars
                    and _clean_len(part2) + seg_clean_len <= max_chars
                ):
            ...
            if not cut_done:
                ...
                if _strip_escaped_sml(combined).strip():
                    ...
                    if seg_clean_len > max_chars:
                        ...
                        buffer = (
                            [pending, seg_content] if pending.strip() else [seg_content]
                        )
                        current_len = seg_clean_len
        ...
        )  # max_chars + overhead of max_chars/2
...
def get_date_entities(
    text: str, stanza_nlp: Pipeline
    ...
    def _clean_single_num(num_str: str) -> str:
        ...
        if is_num2words_compat:
            ...
            phoneme_map = language_math_phonemes.get(
                lang,
                language_math_phonemes.get(
                    default_language_code, language_math_phonemes["eng"]
                ),
            )
...
def year2words(
    year_str: str, lang: str, lang_iso1: str, is_num2words_compat: bool
) -> str | bool:
    try:
        year = int(year_str)
        ...
        lang_iso1 = (
            lang_iso1
            if lang in language_math_phonemes.keys()
        ...
        if not year_str.isdigit() or len(year_str) != 4 or last_two < 10:
            if is_num2words_compat:
                ...
                return " ".join(
                    language_math_phonemes[lang].get(ch, ch) for ch in year_str
                )
        if is_num2words_compat:
            return f"{num2words(first_two, lang=lang_iso1)} {num2words(last_two, lang=lang_iso1)}"
        else:
            return (
                " ".join(language_math_phonemes[lang].get(ch, ch) for ch in first_two)
                + " "
                + " ".join(language_math_phonemes[lang].get(ch, ch) for ch in last_two)
            )
    except Exception as e:
...
def clock2words(
    text: str, lang: str, lang_iso1: str, tts_engine: str, is_num2words_compat: bool
) -> str:
    ...
    def _repl_num(m: re.Match) -> str:
        ...
        if mnt == 0 and (sec is None or sec == 0):
            ...
            phrase = (
                lc["past"].format(hour=_n2w(h), minute=_n2w(mnt))
                if mnt != 0
                else lc["oclock"].format(hour=_n2w(h))
            )
        else:
    ...
    time_rx = re.compile(r"\b([01]?\d|2[0-3]):([0-5]\d)(?::([0-5]\d))?\b")
...
def math2words(
    ...
    # Symbol phonemes
    ambiguous_symbols = {"-", "/", "*", "x"}
    phonemes_list = language_math_phonemes.get(
        lang, language_math_phonemes[default_language_code]
    )
    replacements = {
        k: v
        for k, v in phonemes_list.items()
        if not k.isdigit() and k not in [",", "."]
    }
    normal_replacements = {
        k: v for k, v in replacements.items() if k not in ambiguous_symbols
    }
    ambiguous_replacements = {
        k: v for k, v in replacements.items() if k in ambiguous_symbols
    }
    ...
    if normal_replacements:
        sym_pat = (
            r"("
            + "|".join(
                re.escape(k) for k in sorted(normal_replacements, key=len, reverse=True)
            )
            + r")"
        )
...
def roman2number(text: str) -> str:
    ...
    text = re.sub(
        r"^(?:\s*)([IVXLCDM]+)([.-])(\s+)", _repl_heading, text, flags=re.MULTILINE
    )
    text = re.sub(
        r"^(?:\s*)([IVXLCDM]+)([.-])(?:\s*)$",
        _repl_standalone,
    ...
    text = re.sub(r"(?<!\S)([IVXLCDM]{2,})(?!\S)", _repl_word, text)
    return text


def is_latin(s: str) -> bool:
    return all(
        ("a" <= ch.lower() <= "z") or ch.isdigit() or not ch.isalpha() for ch in s
    )
        ...
        if info.get("paired"):
            if close:
                if not stack or stack[-1] != tag:
                    error = (
                        f"normalize_sml_tags() error: unmatched closing tag [/{tag}]"
                    )
                    return False, error
                stack.pop()
                out.append(f"[/{tag}]")
            ...
            if close:
                error = (
                    f"normalize_sml_tags() error: non-paired tag [/{tag}] is invalid"
                )
                return False, error
...
def convert_chapters2audio(session_id: str) -> bool:
    ...
    def _reset_chapter_file(block_id: str) -> None:
        ch_file = os.path.join(
            session["chapters_dir"], f"{block_id}.{default_audio_proc_format}"
        )
        ...
        for j in valid_idx:
            ...
            if (not is_sml) or (j == last_idx):
                sentence_file = os.path.join(
                    block_dir, f"{j}.{default_audio_proc_format}"
                )
    ...
    try:
        ...
        prev_blocks = (
            {b["id"]: b for b in prev_blocks_list}
            if isinstance(prev_blocks_list, list)
            else prev_blocks_list
        )
        xtts_languages = default_engine_settings[TTS_ENGINES["XTTS"]].get(
            "languages", {}
        )
        ...
        with tqdm(
            ...
            for x, block in enumerate(blocks):
                ...
                start_sentence = 0
                chapter_audio_file = os.path.join(
                ...
                show_alert(
                    ...
                    {
                        "type": "info",
                        "msg": f"Chapter {ch_num} (block {x}) containing {block_len} sentences…",
                    },
                )
                ...
                for j in range(block_len):
                    ...
                    if j in valid_idx:
                        if j >= start_sentence or j in missing_sentences:
                            ...
                            run, error = tts_manager.convert_sentence2audio(
                ...
                show_alert(
                    session_id,
                    {"type": "info", "msg": f"End of Chapter {ch_num} (block {x})"},
                )
                if converted or block_changed or missing_sentences:
                    ...
                    save_db_stamp(session_id)
                    last_save_time = time.monotonic()
                    if not combine_audio_sentences(
                        session_id, chapter_audio_file, block_id, block_len
                    ):
                        show_alert(
                            session_id,
                            {
...
def combine_audio_sentences(
    session_id: str, file: str, block_id: str, sentence_count: int
) -> bool:
    try:
        session = context.get_session(session_id)
        if not session or not session.get("id", False):
    ...
    def _generate_ffmpeg_metadata(
        ...
        try:
            ...
            if session["metadata"].get("language"):
                ffmpeg_metadata += (
                    f"{tag('language')}={session['metadata']['language']}\n"
                )
            if session["metadata"].get("description"):
                ffmpeg_metadata += (
                    f"{tag('description')}={session['metadata']['description']}\n"
                )
            ...
            progress_desc = (
                f"Metadata Part {part_num}" if part_num is not None else "Metadata"
            )
            iterable = (
                part_chapters
                if is_gui_process
                else tqdm(part_chapters, desc=progress_desc, total=total, unit="ch")
            )
            for i, (filename, chapter_title) in enumerate(iterable):
                ...
                duration_ms = len(
                    AudioSegment.from_file(filepath, format=default_audio_proc_format)
                )
                clean_title = re.sub(
                    r"(^#)|[=\\]|(-$)",
                    lambda m: "\\" + (m.group(1) or m.group(0)),
                    sanitize_meta_chapter_title(chapter_title),
                )
    ...
    def _export_audio(
        ...
        try:
            ...
            ffprobe_cmd = [
                ...
                "a:0",
                "-show_entries",
                "stream=codec_name,sample_rate,sample_fmt",
                "-of",
            ...
            if input_codec == target_codec and input_rate == target_rate:
                ...
                cmd += [
                    ...
                    "0",
                    "-af",
                    "dynaudnorm=f=150:g=15,afftdn=nf=-70",
                    "-threads",
                    "0",
                    "-progress",
                    "pipe:2",
            ...
            progress_desc = (
                f"Export Part {part_num}" if part_num is not None else "Export"
            )
            ...
            if not (os.path.exists(final_file) and os.path.getsize(final_file) > 0):
                ...
                return False
            if (
                session["output_format"] in ["mp3", "m4a", "m4b", "mp4"]
                and session["cover"] is not None
            ):
            ...
            final_vtt = os.path.join(
                session["audiobooks_dir"], f"{Path(final_file).stem}.vtt"
            )
            vtt_built, error = build_vtt_file(
    ...
    try:
        ...
        for x, block in enumerate(session["blocks_current"]["blocks"]):
            ...
            if not block.get("sentences"):
                error = (
                    f"Block {x} (id {block['id']}) has no sentences but is marked keep"
                )
                print(error)
                return None
            block_id = block["id"]
        ...
        if session.get("output_chapter_mode"):
            ...
            for idx, (fname, title) in enumerate(zip(chapter_files, chapter_titles)):
                ...
                chapter_file = os.path.join(
                    ...
                    f"{stem}_Ch{idx + 1:0{pad_width}d}.{session['output_format']}",
                )
                if (
                    os.path.exists(chapter_file)
                    and os.path.getsize(chapter_file) > 0
                    and not session.get("output_overwrite")
                ):
                    exported_files.append(chapter_file)
                    continue
                metadata_file = os.path.join(
                    session["process_dir"], f"metadata_ch{idx + 1:0{pad_width}d}.txt"
                )
                _generate_ffmpeg_metadata(
                    [(fname, title)], metadata_file, default_audio_proc_format
                )
                ...
                if _export_audio(
                    ...
                    metadata_file,
                    chapter_file,
                    block_indices={chapter_positions[idx]},
                ):
                    exported_files.append(chapter_file)
            ...
            for part_idx, (part_file_list, indices) in enumerate(
                zip(part_files, part_chapter_indices)
            ):
                concat_list = os.path.join(
                    concat_dir, f"concat_list_chapters_{part_idx + 1:0{pad_width}d}.txt"
                )
                with open(concat_list, "w") as f:
                ...
                merged_audio = (
                    ...
                    / f"{get_sanitized(session['metadata']['title'])}_part{part_idx + 1:0{pad_width}d}.{default_audio_proc_format}"
                )
                result = assemble_audio_chunks(
                    concat_list, merged_audio, is_gui_process
                )
                if not result:
                ...
                metadata_file = (
                    Path(session["process_dir"])
                    / f"metadata_part{part_idx + 1:0{pad_width}d}.txt"
                )
                part_chapters = [(chapter_files[i], chapter_titles[i]) for i in indices]
                _generate_ffmpeg_metadata(
                    part_chapters, str(metadata_file), default_audio_proc_format
                )
                final_file = os.path.join(
                    session["audiobooks_dir"],
                    f"{Path(session['final_name']).stem}_part{part_idx + 1:0{pad_width}d}.{session['output_format']}"
                    if is_multi_part
                    else session["final_name"],
                )
                block_indices = (
                    {chapter_positions[i] for i in indices} if is_multi_part else None
                )
                if _export_audio(
                    merged_audio,
                    metadata_file,
            ...
            if (
                existing_count > 0
                and existing_count < len(chapter_files)
                and not session.get("output_overwrite")
            ):
                ...
                new_audio = (
                    Path(session["process_dir"])
                    / f"{get_sanitized(session['metadata']['title'])}_new.{default_audio_proc_format}"
                )
                ...
                _generate_ffmpeg_metadata(
                    chapters_zip, metadata_file, default_audio_proc_format
                )
                ...
                if not result:
                    print(
                        f"assemble_audio_chunks() Final merge failed for {merged_audio}."
                    )
                    return None
                metadata_file = os.path.join(session["process_dir"], "metadata.txt")
                chapters_zip = list(zip(chapter_files, chapter_titles))
            ...
            with open(txt_file, "r") as f:
                for line in f:
                    if line.strip().startswith("file"):
                        file_path = (
                            line.strip().split("file ")[1].strip().strip("'").strip('"')
                        )
                    ...
                    if dir in current_user_dirs:
                        ...
                        if os.path.isdir(full_dir_path):
                            try:
                                ...
                                if (
                                    dir_mtime < threshold_time
                                    and dir_ctime < threshold_time
                                ):
        ...
        with tqdm(total=total, desc="translate", unit="block") as t:
            for idx, text in enumerate(raw_blocks):
                ...
                if progress_bar is not None:
                    progress_bar(
                        (t.n) / total,
                        desc=f"Translating block {t.n}/{total} {source_iso1} -> {target_iso1}",
                    )
...
def convert_ebook(args: dict) -> tuple:
    try:
        ...
        if args["language"] is not None:
            ...
            translate_to = (
                args.get("translate")
                if args.get("translate") != args.get("language")
                else False
            )
            translate_enabled = bool(args.get("translate_enabled")) and bool(
                translate_to
            )
            ...
            if session["ebook_mode"] == ebook_modes["TEXT"]:
                ...
                text_name_hash = hashlib.md5(
                    f"{text_name}_{session_id}".encode()
                ).hexdigest()
            ...
            session["custom_model_dir"] = os.path.join(
                models_dir, "__sessions", f"model-{session_id}"
            )
            session["script_mode"] = (
                str(args["script_mode"])
                if args.get("script_mode") is not None
                else NATIVE
            )
            session["is_gui_process"] = bool(args["is_gui_process"])
            session["blocks_preview"] = (
                bool(args["blocks_preview"]) if args.get("blocks_preview") else False
            )
            ...
            session["xtts_enable_text_splitting"] = bool(
                args["xtts_enable_text_splitting"]
            )
            ...
            if session["is_gui_process"]:
                ...
                session["audiobooks_dir"] = (
                    ...
                    else os.path.join(audiobooks_cli_dir, f"cli-{session_id}")
                )
                session["final_name"] = os.path.join(
                    session["audiobooks_dir"],
                    ebook_name
                    + ("_" + language if session.get("translate_enabled") else "")
                    + "."
                ...
                if error is None:
                    ...
                    if session["custom_model"] is not None:
                        ...
                        if not os.path.exists(
                            os.path.join(session["custom_model_dir"], custom_src_name)
                        ):
                            try:
                    ...
                    if session.get("voice"):
                        voice_name = os.path.splitext(
                            os.path.basename(session["voice"])
                        )[0].replace("&", "And")
                        ...
                        if not os.path.exists(final_voice_file):
                            extractor = VoiceExtractor(
                                session, session["voice"], voice_name
                            )
            ...
            if error is None:
                if prepare_dirs(session_id):
                    ...
                    vram_dict = VRAMDetector().detect_vram(
                        session["device"], session["script_mode"]
                    )
                    ...
                    if session["free_vram_gb"] == 0:
                        ...
                        msg_extra += (
                            f"<br/>Free Memory available: {session['free_vram_gb']}GB"
                        )
                        if (
                            session["free_vram_gb"]
                            < default_engine_settings[session["tts_engine"]]["rating"][
                                "VRAM"
                            ]
                        ):
                    ...
                    device_vram_required = (
                        default_engine_settings[session["tts_engine"]]["rating"]["RAM"]
                        if session["device"] == devices["CPU"]["proc"]
                        else default_engine_settings[session["tts_engine"]]["rating"][
                            "VRAM"
                        ]
                    )
                    if float(total_vram_gb) >= float(device_vram_required):
                        if msg:
                            show_alert(
                                session_id, {"type": "warning", "msg": msg + msg_extra}
                            )
                        else:
                            show_alert(session_id, {"type": "info", "msg": msg_extra})
                        session["epub_path"] = os.path.join(
                            session["process_dir"],
                            f"__{session['filename_noext']}.epub",
                        )
                        session["blocks_orig_json"] = os.path.join(
                            session["process_dir"],
                            f"{file_prefixes['clone']}{session['filename_noext']}.json",
                        )
                        session["blocks_saved_json"] = os.path.join(
                            session["process_dir"],
                            f"{file_prefixes['saved']}{session['filename_noext']}.json",
                        )
                        session["blocks_current_db"] = os.path.join(
                            session["process_dir"],
                        ...
                        if error is None:
                            ...
                            if os.path.exists(session["blocks_orig_json"]):
                                ...
                                if blocks_orig:
                                    ...
                                    for block in blocks:
                                        if any(
                                            c.isalnum() for c in block.get("text", "")
                                        ):
                                            if not block.get("id"):
                            ...
                            if epubBook:
                                ...
                                metadata["title"] = metadata["title"] or Path(
                                    session["ebook"]
                                ).stem.replace("_", " ")
                                metadata["creator"] = (
                                    False
                                ...
                                if not session.get("translate_enabled"):
                                    if (
                                        ...
                                        show_alert(
                                            session_id,
                                            {"type": "warning", "msg": error},
                                        )
                                is_lang_in_tts_engine = (
                                    session.get("tts_engine") in default_engine_settings
                                    and language
                                    in default_engine_settings[
                                        session["tts_engine"]
                                    ].get("languages", {})
                                )
                                if is_lang_in_tts_engine:
                                    ...
                                    if session.get("cover", False):
                                        if missing_orig_json:
                                            ...
                                            if raw_blocks and session.get(
                                                "translate_enabled"
                                            ):
                                                raw_blocks, error = translate_blocks(
                                                    session_id, list(raw_blocks)
                                                )
                                            ...
                                            if session.get("blocks_orig", {}):
                                                save_json_blocks(
                                                    session_id, "blocks_orig"
                                                )
                                        if not session.get("blocks_current", {}):
                                            session["blocks_current"] = copy.deepcopy(
                                                session["blocks_orig"]
                                            )
                                            save_db_blocks(session_id)
                                        ...
                                        if session.get(
                                            ...
                                            if session["blocks_preview"]:
                                                ...
                                                print(msg)
                                                progress_status = os.path.basename(
                                                    session["ebook"]
                                                )
                                                return progress_status, True
                                            else:
                                                progress_status, passed = (
                                                    finalize_audiobook(session_id)
                                                )
...
def finalize_audiobook(session_id: str) -> tuple:
    try:
        ...
        result = lambda msg, ok: (
            (gr.update(value=msg), gr.update(value=ok)) if is_preview else (msg, ok)
        )
        ...
        if not conversion:
            ...
            return _fail(error)
        show_alert(
            session_id, {"type": "info", "msg": "Combining sentences and chapters…"}
        )
        ...
        # Upload to Audiobookshelf if enabled
        if session.get("abs_enabled", False):
            abs_file = exported_files[-1]
            abs_title = Path(abs_file).stem
            ...
            if abs_server_url and abs_api_token and abs_library_id:
                try:
                    ...
                    show_alert(
                        session_id,
                        {"type": "error", "msg": f"Audiobookshelf upload failed: {e}"},
                    )
        count_ebook = 0
        ...
        if count_ebook > 0:
            ...
            show_alert(
                ...
                {
                    "type": "success",
                    "msg": f"{filename} / converted. {count_ebook} ebook(s) conversion remaining…",
                },
            )
        else:
            if session["ebook_mode"] == ebook_modes["DIRECTORY"]:
            ...
            reset_ebook_session(session_id, force=True, filter_keys=False)
            show_alert(
                session_id, {"type": "success", "msg": f"{filename} / converted."}
            )
            print(f"*********** Session: {session_id} **************\n{session_info}")
...
def restore_session_from_data(
    data: dict, session: DictProxy, force: bool, filter_keys: bool
) -> None:
    try:
        for key, value in data.items():
            if key in session:
...
def cleanup_models_cache() -> None:
    try:
        active_models = {
            ...
            for cache in (
                session.get("model_cache"),
                session.get("model_zs_cache"),
                session.get("stanza_cache"),
            )
